...
export const selectRandomRudiments = (categories, numberOfRudiments) => {
  ...
  const runSwitchStatement = () => {
    ...
    // Edge case: When only the "Diddle Rudiments" category is selected, the count should be limited to 4 since there are no more than 4 diddle rudiments in the data set.
  ...
  if (indicesSelected.length <= numberOfRudiments) {
    for (let i = 0; i < 6; i++) {
      ...
      // Generate even more indices so that when category2 is selected the odds of randomly generating all four options are statistically much higher to avoid a stack overflow.
    }
  }
      ...
      for (let i = 0; i < numberOfRudiments; i++) {
        const selectIndexFromSet = () => {
          ...
          if (finalIndicesSelected.length === numberOfRudiments) {
            ...
            return selectIndexFromSet(); // Recursive function call.
          }
        };
        selectIndexFromSet();
      }
